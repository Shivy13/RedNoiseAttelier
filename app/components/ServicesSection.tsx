...
export const ServiceSection: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uWispDensity: new THREE.Uniform(wispDensity),
      uTiltScale: new THREE.Uniform(mouseTiltStrength),
      uFlowTime: new THREE.Uniform(0),
      uFogTime: new THREE.Uniform(0),
      uBeamXFrac: new THREE.Uniform(horizontalBeamOffset),
      uBeamYFrac: new THREE.Uniform(verticalBeamOffset),
      uFlowSpeed: new THREE.Uniform(flowSpeed),
      uVLenFactor: new THREE.Uniform(verticalSizing),
      uHLenFactor: new THREE.Uniform(horizontalSizing),
      uFogIntensity: new THREE.Uniform(fogIntensity),
      uFogScale: new THREE.Uniform(fogScale),
      uWSpeed: new THREE.Uniform(wispSpeed),
      uWIntensity: new THREE.Uniform(wispIntensity),
      uFlowStrength: new THREE.Uniform(flowStrength),
      uDecay: new THREE.Uniform(decay),
      uFalloffStart: new THREE.Uniform(falloffStart),
      uFogFallSpeed: new THREE.Uniform(fogFallSpeed),
      uColor: new THREE.Uniform(new THREE.Vector3(1, 1, 1)),
      uFade: new THREE.Uniform(hasFadedRef.current ? 1 : 0)
    };
  ...
  return (
    <>
      <div>
        {/* Heading - Staggered line animation */}
        <motion.div ref={headingRef} className="uppercase text-center mb-0 md:mb-20" style={{marginBottom:'-3.5vh'}}>
          {["What we bring ", "to the table"].map((line, i) => (
            <motion.h2
              ...
              animate={
                isHeadingInView ? { opacity: 1, x: 0 } : { opacity: 0, x: -30 }
              }
              transition={{ duration: 0.6, delay: i * 0.2 }}
            >
              {/* color the first letter of the word "table" in red */}
              {line === "to the table" ? (
                <>
      ...
      <div ref={mountRef} className={`w-full h-[100vh] relative ${className || ''}`} style={style} />
      <Services/>
      </div>
    </>
  );
